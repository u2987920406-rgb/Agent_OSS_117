...
export function startWebScraperAgent() {
  eventBus.on(EVENT_CHANNELS.TASK_CREATED, (taskId: string) => {
    ...
    if (!url) {
      const urlMatch = payload.instruction.match(/https?:\/\/[^\s"'<'>]+/);
      url = urlMatch ? urlMatch[0] : null;
    }

    if (!url) {
      emitLog("WebScraper", "error", "Aucune URL trouvee.");
      eventBus.emit(EVENT_CHANNELS.TASK_COMPLETED, taskId, "Erreur: Aucune URL trouvee dans la requete.");
      return;
    }
